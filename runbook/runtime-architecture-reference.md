...
Define the module separation, queue topology, and message lifecycle that every WA Agent runtime should follow. Technology choices (Node.js, Python, Go) are implementation details — the architecture patterns are universal.
...
3. Classify `fromMe` events (outbox echo vs human admin)
4. Enqueue the normalized event
5. Return 200

It must NOT call the LLM, call the WhatsApp bridge, or do business logic.
...
The planner never calls the WhatsApp bridge directly. The outbox worker handles all sends.
...
Never call the WhatsApp bridge inline in the webhook handler:
...
Handoff trigger → admin notification → same-number human activity detection → pause → continuity summary → resume. AI must never fight for keyboard. If owner is `human`, AI may update memory but must not send.
...
Queue: outbox-send         FIFO per conversation_id, delayed by pacing  (WhatsApp send)
...
WhatsApp → Bridge webhook
  ↓
Gateway: parse, validate, idempotency check, fromMe classify
  ↓
...
|---|---|---|
| WhatsApp bridge | Evolution API v2 (Baileys) | Official Meta Cloud API |
| Persistence | Postgres / Supabase | Self-hosted + PgBouncer |
...
Do not add infrastructure complexity before the correctness work is complete (state machine, dedupe, handoff, fromMe classifier).
