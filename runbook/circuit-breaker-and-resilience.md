...
Define the resilience patterns every WA Agent runtime must implement to survive production failures — LLM provider outages, Evolution API unreliability, webhook floods, and worker crashes.
...
When the WhatsApp bridge rejects or times out a send:
...
When the runtime's webhook endpoint has high latency, the WhatsApp bridge will re-deliver the same webhook. Combined with idempotency, this is safe. But under extreme load, the retry flood itself can overwhelm the system.
