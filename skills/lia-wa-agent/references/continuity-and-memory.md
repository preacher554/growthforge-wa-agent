...
- What the next action the customer is expecting is

Without a continuity summary, the AI will contradict what the admin said. In Indonesian SMB conversations where negotiation is common, this destroys customer trust immediately.
...
-- tenant_id + customer_jid together = isolated per tenant
```
...
Stored in tenant config files and loaded by `tenant.loader.ts` at conversation start.
...
CORRECT: customer_memories WHERE tenant_id = 'tenant_xyz' AND customer_jid = '+62812xxxx'
```
