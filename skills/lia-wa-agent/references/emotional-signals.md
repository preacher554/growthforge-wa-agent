...
Emotional state must be a runtime record, not just an instruction in a prompt.

## Signal types

| signal_type | signal_value options | When to write |
|---|---|---|
| `emotional_state` | angry, frustrated, confused, neutral, positive, urgent | After each AI planning step when LLM reasoning contains emotional inference |
| `urgency` | immediate, high, medium, low | When message contains time-sensitive language |
| `lead_temperature` | hot, warm, cold, unknown | After lead qualification step in Pro package |
...
After each AI planning step, the planner extracts signals from the LLM reasoning output:
...
These fire on `message.received` before buffering. If an urgency keyword is detected, set urgency signal immediately and shorten buffer flush time to 0ms (flush immediately).
...
| `emotional_state = confused` for 3+ consecutive turns | Suggest simpler language in next AI reply (passed as context hint) |
...
These become value metrics for the GrowthForge managed service. Tenants see that the AI is not just answering — it is tracking lead quality and customer sentiment.
