...
## Why a circuit breaker is required

When the LLM provider goes down, rate-limits, or times out, a naive retry loop does three things: exhausts the queue, floods the provider with requests that will also fail, and leaves customers waiting indefinitely with no communication. A circuit breaker stops this cascade at the first sign of systemic failure.
...
```text
1. Move to dead-letter queue (BullMQ failed queue)
...
Track the circuit state in Redis:
```text
...
For v1 single-tenant managed deployments, a full per-tenant circuit breaker is overkill. Implement the retry + auto-escalation flow first. Add circuit breaker state tracking when serving 20+ concurrent active tenants.

## Outbox send failure handling

When Evolution API rejects or times out a send:
...
When the runtime's webhook endpoint has high latency, Evolution (and Meta) will re-deliver the same webhook. Combined with idempotency, this is safe. But under extreme load, the retry flood itself can overwhelm the system.

```text
Ingress gateway hardening:
  1. Idempotency check (Redis SETNX) must complete in < 5ms
...
Redis SETNX pattern for wamid deduplication:
```text
...
- [ ] Redis SETNX idempotency check completes in < 5ms
- [ ] Webhook handler returns 200 in < 200ms regardless of downstream state
- [ ] Stale lock recovery worker runs on schedule
- [ ] Circuit breaker state tracked per provider in Redis for scale deployments
