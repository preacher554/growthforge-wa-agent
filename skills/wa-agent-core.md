...
Shared operating skill for all WA Agent packages (Basic, Pro, Custom/Add-on).

## Core model

WA Agent is a managed WhatsApp AI employee using the client's own WhatsApp number.
...
The WhatsApp bridge emits `key: { fromMe: true }` when the admin replies from the same business number. The runtime MUST:
